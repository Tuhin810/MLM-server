...
export class WalletController {
    ...
    async deposit(req, res, next) {
        try {
            ...
            const depositSchema = z.object({
                amount: z.preprocess((val) => parseFloat(val), z.number().positive("Deposit amount must be positive")),
            });
            const { amount } = depositSchema.parse(req.body);
            const updatedWallet = await walletRepository.updateBalanceAndPoints(req.user.id, amount, 0, TransactionType.CREDIT, "Wallet deposit (Simulated Payment)");
            res.status(200).json({
    ...
    async submitFranchiseFundRequest(req, res, next) {
        try {
            if (!req.user) {
                res.status(401).json({ error: "Unauthorized" });
                return;
            ...
            const schema = z.object({
                amount: z.preprocess((val) => parseFloat(val), z.number().positive("Amount must be positive")),
