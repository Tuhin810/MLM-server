...
export class AdminService {
  ...
  async getUsersPaginated({ page, limit, search }: { page: number; limit: number; search?: string }) {
  ...
  async getReferralUsers() {
    ...
    const usersWithDirectReferralsCount = await Promise.all(
  ...
  async getReferralTree(userId: string) {
    const getTree = async (uId: string, currentLevel: number = 0, maxLevel: number = 4): Promise<any> => {
      const user = await prisma.user.findUnique({
        where: { id: uId },
        select: {
          id: true,
