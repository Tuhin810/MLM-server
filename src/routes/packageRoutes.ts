...
router.post("/:id/subscribe", authMiddleware as any, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (existing) { res.status(400).json({ error: "Already subscribed to this package" }); return; }
    ...
    await walletRepo.updateBalanceAndPoints(req.user.id, -pkg.price, pkg.pointValue, "DEBIT" as any, `Package subscription: ${pkg.name}`);
...
router.get("/my", authMiddleware as any, async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
  try {
    if (!req.user) { res.status(401).json({ error: "Unauthorized" }); return; }
